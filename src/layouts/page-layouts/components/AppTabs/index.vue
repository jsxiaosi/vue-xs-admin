...
const { multiTabs: storeMultiTabs, MultiTabsDropReordering } =
...
const { setTabPaneKey, addRouteTabs, onFresh, removeTab } =
...
watch(
  ...
  async () => {
    addRouteTabs(
      route.matched[route.matched.length - 1] as unknown as MultiTabsType,
    );
...
const elDropdownRef =
  useTemplateRef<InstanceType<typeof ElDropdown>>("el-dropdown-ref");
...
const initTableDrag = () => {
  if (!appConfig.value.closeTabDrag)
    initSortable(
      document.querySelector<HTMLElement>(".tabs-container .el-tabs__nav"),
    );
};
