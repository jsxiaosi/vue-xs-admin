...
export const useTabsChange = (multiTabs: Ref<MultiTabsType[]>) => {
  ...
  const setTabPaneKey = (
    item: MultiTabsType | RouteLocationNormalizedLoaded,
  ): string => {
  ...
  const closeTabsRoute = (
    ...
    const item = multiTabs.value.findIndex(
      (i) => setTabPaneKey(i) === setTabPaneKey(e),
    );
  ...
  function onFresh(item?: MultiTabsType) {
    ...
    setTimeout(() => {
      removeClass(
        document.querySelector(".rotate") as HTMLElement,
        refreshButton,
      );
    }, 600);
  }
